...
class FuzzyInertiaController_5L:
    """
    Mamdani FIS (5 labels: very_low, low, medium, high, very_high):
    ...
    def _build_w_mfs(self, w_set):
        ...
        W_SETS = {
            "O1": {
                ...
                "very_low": (0.0, 0.0, 0.33),
                "low":      (0.17, 0.33, 0.5),
                "medium":   (0.33, 0.5, 0.67),
                "high":     (0.50, 0.67, 0.83),
                "very_high":(0.67, 1.0, 1.0),
            # },
            # "O2": {
    ...
    def _build_rules_5x5(self, rule_set):
        """
        WEA2026: 8 variantes de bases de reglas 5×5 (25 reglas cada una).
        Rows = diversity (very_low..very_high), Columns = progress (very_early..very_late).
        
        Diseño factorial 2×2 (R1, R6, R7, R8):
        Diseño factorial ortogonal 2×3:
                            Div-agnostic(—)  Div-following(→)  Div-compensating(←)
        Progress ↓ (VH→VL): R1               R5                R6
        Progress ↑ (VL→VH): R2               R7                R8
        Progress — (flat):   (not useful)     R3                R4

        Additive model: output = clip(progress_effect + diversity_effect)
        5L progress: very_early=+2, early=+1, mid=0, late=-1, very_late=-2
        5L diversity: very_low=±2, low=±1, medium=0, high=±1, very_high=±2
        ...
        """
        RULE_SETS = {
            # R1: Progress ↓, diversity-agnostic
            ...
            # R2: Progress ↑, diversity-agnostic (mirror of R1)
            # All rows identical: VL → L → M → H → VH
            "R2": {
                ...
                ("low", "mid"):             "medium",
                ("low", "late"):            "high",
                ("low", "very_late"):       "very_high",
                ("medium", "very_early"):   "very_low",
                ("medium", "early"):        "low",
                ("medium", "mid"):          "medium",
                ("medium", "late"):         "high",
                ("medium", "very_late"):    "very_high",
                ("high", "very_early"):     "very_low",
                ("high", "early"):          "low",
                ("high", "mid"):            "medium",
                ("high", "late"):           "high",
            ...
            # R3: Diversity-following, progress-agnostic (mirror of R4)
            # All columns identical: VL → L → M → H → VH
            "R3": {
                ("very_low", "very_early"): "very_low",
            ...
            # R4: Diversity-compensating, progress-agnostic (mirror of R3)
            # All columns identical: VH → H → M → L → VL
            "R4": {
                ("very_low", "very_early"): "very_high",
                ("very_low", "early"):      "very_high",
                ("very_low", "mid"):        "very_high",
                ("very_low", "late"):       "very_high",
                ("very_low", "very_late"):  "very_high",
                ("low", "very_early"):      "high",
                ("low", "early"):           "high",
                ("low", "mid"):             "high",
            ...
            # R5: Progress ↓ + diversity-following (mirror of R8)
            # prog: VE=+2,E=+1,mid=0,L=-1,VL=-2; div: VL=-2,L=-1,M=0,H=+1,VH=+2
            "R5": {
                ...
                ("very_high", "mid"):       "high",
                ("very_high", "late"):      "high",
                ("very_high", "very_late"): "medium",
            },
            # R6: Progress ↓ + diversity-compensating (mirror of R7)
            # prog: VE=+2,E=+1,mid=0,L=-1,VL=-2; div_comp: VL=+2,L=+1,M=0,H=-1,VH=-2
            "R6": {
                ("very_low", "very_early"): "very_high",
                ("very_low", "early"):      "very_high",
                ("very_low", "mid"):        "high",
            ...
            # R7: Progress ↑ + diversity-following (mirror of R6)
            # prog_inv: VE=-2,E=-1,mid=0,L=+1,VL=+2; div: VL=-2,L=-1,M=0,H=+1,VH=+2
            "R7": {
                ("very_low", "very_early"): "very_low",
                ("very_low", "early"):      "very_low",
                ("very_low", "mid"):        "low",
            ...
            # R8: Progress ↑ + diversity-compensating (mirror of R5)
            # prog_inv: VE=-2,E=-1,mid=0,L=+1,VL=+2; div_comp: VL=+2,L=+1,M=0,H=-1,VH=-2
            "R8": {
                ("very_low", "very_early"): "medium",
                ("very_low", "early"):      "high",
                ("very_low", "mid"):        "high",
